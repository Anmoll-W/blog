...
- "roughly eighteen Mac scripts to a handful of jobs" — the Wave-2 consolidation inventory (~18 Mac runners to the new server jobs).
- The five catches (deploy-can't-deploy-its-own-repo; fail-open backup causing a target wipe on rollback; synced idempotency ledger causing a re-fire; consolidated-job double-send on retry; two false-green tests caught by mutation-testing the tests) — each reproduced, fixed, and mutation-verified this session; recorded in the Hermes decisions log as outcomes, not plan intent.
