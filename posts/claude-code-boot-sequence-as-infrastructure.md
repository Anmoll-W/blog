...
- [I Cherry-Picked a Viral Cost-Cut Post](cherry-picking-the-cost-post.md) — the four token-efficiency rules that landed inside this same `~/.claude/CLAUDE.md` infrastructure, plus the bug an eval agent caught when one rule was imported from a different billing tier.
- [The Boot Hook That Refired on Every Compaction](the-boot-hook-that-refired-on-compaction.md) — the SessionStart hook built here had one blind spot left: it never checked WHY a session was starting, and compaction counts as a start
