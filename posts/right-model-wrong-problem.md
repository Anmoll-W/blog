...
- [What the Model Should Not Decide](what-the-model-should-not-decide.md): the question before "which model", namely whether a model belongs in the decision at all, and what to route to deterministic code instead
- [Most AI Builds Start at Step Four. Here Is the Order I Default To.](the-order-you-build-ai-in.md): the general framework this session is a live instance of. Picking the approach before naming the customer, problem, and workflow is the failure mode, and the question that saved this build is the one the order forces you to ask
