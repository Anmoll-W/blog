...
- [What the Model Should Not Decide](what-the-model-should-not-decide.md): the framework behind this pipeline's core choice, why automating everything up to the click is an autonomy rung rather than a compromise, and why tool and action guardrails matter more than the visible ones.
- [Most AI Builds Start at Step Four. Here Is the Order I Default To.](the-order-you-build-ai-in.md): the framework that would have predicted both findings here, because it treats untrusted text and tool scope as design inputs at the guardrail stage rather than as an adversarial pass afterwards.
