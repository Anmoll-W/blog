...
- "nothing auto-posts; human edits raw→ready and posts manually; enforced by executable guard test (no posting capability, hardcoded status: raw, single-writer)" — design Invariants 1+3; session MW3-4 test_no_autopost.sh (8 tests, commit 2385ab6)
...
- INJECTION HOLES caught by two-reviewer gate-3, each reproduced+fixed+regression-tested:
  ...
  (c) hash/fingerprint collisions silently drop content — session MW3-1 "unescaped | → NUL separator" + "undated analyst report → empty-date cross-file collision → key on filename stem" BLOCKERs
- SEPARATE end-to-end review caught: duplicate-draft-on-retry (session next-steps MW3-2 crash-window note: write→fsync→append atomic ledger; "duplicate-draft" idempotency) + over-broad sync rule copying private session notes (session "slice-widen too broad" → corrected to name exactly Knowledge/decisions.md + Projects/PM-Code/Posts/Drafts/ only; staged then applied with validate.sh 63/63)
